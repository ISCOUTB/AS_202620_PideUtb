...
def test_crear_pedido_exitoso():
    respuesta = client.post("/v1/pedidos", json={"item_id": 1, "cantidad": 2})

    assert respuesta.status_code == 201
    cuerpo = respuesta.json()
    assert cuerpo["nombre_item"] == "Arepa de huevo"
    assert cuerpo["precio_unitario_centavos"] == 400000
    assert cuerpo["cantidad"] == 2
    assert cuerpo["total_centavos"] == 800000
    assert cuerpo["estado"] == "pendiente_pago"


def test_crear_pedido_item_no_encontrado():
    respuesta = client.post("/v1/pedidos", json={"item_id": 999, "cantidad": 1})
...
def test_crear_pedido_item_no_disponible():
    ...
    respuesta = client.post("/v1/pedidos", json={"item_id": 3, "cantidad": 1})
